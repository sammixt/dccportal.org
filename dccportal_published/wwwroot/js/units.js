$(function(){
          ...
          },
          {
            ...
            render: function (data, type, full, meta) {
              return (
                ...
                'Details</a>' +
                `<a href="javascript:;" data-unit="${full['getUnitIdString']}" class="dropdown-item delete-record">` +
    ...
    $("#new-unit-form").on('click','.btn-submit',function(e){
        ...
        var model = {
           ...
           axios.post(`${url}Units/AddUnit`, model)
           .then(response => {
    ...
    $("#unit_list tbody").on('click','.details', function(e){
        ...
        axios.get(`${url}Units/GetUnitDetail?SetUnitIdString=${unit}`)
        .then(response => {
            var d = response.data;
            $("#unit-name").val(d.unitName),
            $("#unit-function").val(d.unitFunction),
            $("#vertical-dept").val(d.deptId),
    ...
    $("#edit-unit-form").on('click','.data-submit',function(e){
        ...
        var model = {
            ...
            deptId: $("#vertical-dept option:selected").val(),
            unitShortCode : $("#unit-shortcode").val(),
            setUnitIdString: $("#setUnitIdString").val()
           }
           axios.put(`${url}Units/EditUnit`, model)
           .then(response => {
                if (response.data.statusCode === 200) {
                    toastr['success'](`👋 ${response.data.message}`, 'Success!', {
                        closeButton: true,
